...
class HerbIdentifierApp:
    def __init__(self, master):
        self.master = master
        master.title("Herb Identifier")

        self.label = Label(master, text="Upload an image of a herb:")
        self.label.pack()

        self.upload_button = Button(master, text="படத்தை பதிவேற்றவும்", command=self.upload_image)
    ...
    def identify_herb(self, image_path):
        ...
        for i in range(len(out)):
            if out[i] == search_name:
                ...
                self.result_label.config(text=f"மூலிகையின் பெயர்: {search_name}\n\nபயன்கள்: \n{uses}")
                break
